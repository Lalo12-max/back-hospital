...
func Register(c *fiber.Ctx) error {
	...
	// Validación de usuario o correo existente
	db := config.GetDB()
	var existingCount int
	err := db.QueryRow("SELECT COUNT(*) FROM Usuarios WHERE email = $1 OR nombre = $2", req.Email, req.Nombre).Scan(&existingCount)
	if err != nil {
	...
	if existingCount > 0 {
		return sendStandardResponse(c, 400, utils.REGISTER_EMAIL_EXISTS, fiber.Map{
			"error":       "Usuario o correo ya existente",
			"description": utils.GetCodeDescription(utils.REGISTER_EMAIL_EXISTS),
		})
	}
	...
	// Eliminar esta línea duplicada: db := config.GetDB()
